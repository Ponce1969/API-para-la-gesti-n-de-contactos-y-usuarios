...
class TokenData(BaseModel):
    ...
    sub: str | None = Field(
        None, description="Identificador del sujeto del token (e.g., email o user_id)"
    )
...
class PasswordResetRequest(BaseModel):
    ...
    email: EmailStr = Field(
        ..., description="Email del usuario para el cual se solicita el reseteo"
    )
...
class ResetPasswordSchema(BaseModel):
    ...
    new_password: str = Field(
        ..., min_length=8, description="Nueva contraseña para el usuario"
    )
