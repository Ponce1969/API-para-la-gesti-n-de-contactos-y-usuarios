...
class TestContactService:
    ...
    async def test_get_contact_by_email_success(self, mock_db, mock_contact):
        ...
        with patch.object(
            ...
            # Act
            result = await ContactService.get_contact_by_email(
        ...
        with patch.object(
            ...
            return_value=Failure(
                ContactNotFoundError(
                    0, "No se encontró un contacto con email test@example.com"
                )
            ),
        ) as mock_get_by_email:
            # Act
            result = await ContactService.get_contact_by_email(
                mock_db, "test@example.com", 1
            )
    ...
    async def test_list_contacts_with_group_id_success(self, mock_db, mock_contact):
        ...
        with (
            ...
            patch.object(
                ...
                "list_contacts",
                return_value=Success(contacts_list),
            ) as mock_list_contacts,
        ):
            # Act
    ...
    async def test_create_contact_success(
        self, mock_db, mock_contact, mock_contact_create
    ):
        # Arrange
        with patch.object(
            ...
            # Act
            result = await ContactService.create_contact(
                mock_db, 1, mock_contact_create
            )
    ...
    async def test_create_contact_already_exists(self, mock_db, mock_contact_create):
        ...
        with patch.object(
            ...
            "create",
            return_value=Failure(
                ContactAlreadyExistsError("juan.perez@example.com", 1)
            ),
        ) as mock_create:
            # Act
            result = await ContactService.create_contact(
    ...
    async def test_update_contact_success(
        self, mock_db, mock_contact, mock_contact_update
    ):
        # Arrange
        with patch.object(
            ...
            result = await ContactService.update_contact(
                mock_db, 1, 1, mock_contact_update
            )
    ...
    async def test_update_contact_not_found(self, mock_db, mock_contact_update):
        # Arrange
        with patch.object(
            ContactRepository, "update", return_value=Failure(ContactNotFoundError(1))
        ) as mock_update:
            # Act
            result = await ContactService.update_contact(
                mock_db, 1, 1, mock_contact_update
