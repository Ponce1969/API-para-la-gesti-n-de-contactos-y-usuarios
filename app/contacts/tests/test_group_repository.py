...
class TestContactGroupRepository:
    ...
    async def test_list_groups_with_search(self, mock_db, mock_group):
        ...
        # Act
        result = await ContactGroupRepository.list_groups(
            mock_db, 1, search="Trab"
        )
    ...
    async def test_update_group_name_conflict(self, mock_db, mock_group):
        ...
        patch.object(
            ContactGroupRepository, 'get_by_name', return_value=Success(existing_group)
        ) as mock_get_by_name:
            # Act
            result = await ContactGroupRepository.update(
                mock_db,
    ...
    async def test_delete_group_success(self, mock_db, mock_group):
        ...
        with patch.object(
            ...
            # Assert
            assert result.is_success()
            assert result.unwrap() is None  # El mu00e9todo delete devuelve None en caso de u00e9xito
            mock_db.delete.assert_called_once_with(mock_group)
            mock_db.flush.assert_called_once()
            mock_get_by_id.assert_called_once()

    async def test_add_contact_to_group_success(self, mock_db, mock_contact, mock_group):
        # Arrange
        mock_execute = AsyncMock()
        mock_db.execute = mock_execute
        ...
        patch.object(
            ...
            assert mock_db.execute.call_count == 2  # Una para verificar y otra para insertar
            mock_db.commit.assert_called_once()

    async def test_add_contact_to_group_already_in_group(self, mock_db, mock_contact, mock_group):
        # Arrange
        mock_execute = AsyncMock()
        mock_db.execute = mock_execute
    ...
    async def test_remove_contact_from_group_success(self, mock_db, mock_contact, mock_group):
        # Arrange
        mock_execute = AsyncMock()
        mock_db.execute = mock_execute
        ...
        patch.object(
            ...
            mock_get_contact.assert_called_once()
            mock_get_group.assert_called_once()
            assert mock_db.execute.call_count == 2  # Una para verificar y otra para eliminar
            mock_db.commit.assert_called_once()

    async def test_remove_contact_from_group_not_in_group(self, mock_db, mock_contact, mock_group):
        # Arrange
        mock_execute = AsyncMock()
        mock_db.execute = mock_execute
