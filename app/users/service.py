...
from app.users.schemas import (  # Añadir esquemas de token
    UserCreate,
    UserInDB,
    UserUpdate,
    VerificationTokenCreateInternal,
    VerificationTokenInDB,
    VerificationTokenServiceCreate,
)
...
class UserService:
    ...
    async def get_user_by_id(
        self, user_id: int
    ) -> Result[UserInDB, UserNotFoundError | DatabaseError]:
    ...
    async def register_new_user(
        ...
        existing_user_by_email_result = await self.user_repository.get_by_email(
            user_data.email
        )
        ...
        if existing_user_by_email_result.is_failure():
            ...
            if not isinstance(error, UserNotFoundError):
                ...
                logger.error(
                    f"Error inesperado al verificar email {user_data.email}: {error}"
                )
        ...
        token_creation_result = await self.create_verification_token_for_user(
            token_service_data
        )
    ...
    async def create_verification_token_for_user(
        ...
        create_token_result = await self.user_repository.create_verification_token(
            repo_token_data
        )
    ...
    async def use_verification_token(
        ...
        token_result = await self.user_repository.get_verification_token_by_value(
            token_value
        )
        ...
        if token_in_db.is_used:
            return Failure(
                TokenInvalidError(
                    token_value=token_value, reason="Token ya ha sido utilizado."
                )
            )
        ...
        if token_in_db.token_type != expected_token_type:
            ...
            return Failure(
                TokenInvalidError(
                    token_value=token_value, reason="Tipo de token incorrecto."
                )
            )
        ...
        if user_result.is_failure():
            ...
            logger.error(
                f"Usuario ID {token_in_db.user_id} no encontrado para token válido ID {token_in_db.id}"
            )
            return user_result
        ...
        updated_user_in_db: UserInDB | None = None

        if expected_token_type == "email_verification":
            if user_to_update.is_verified:
                logger.info(
                    f"Usuario {user_to_update.email} ya estaba verificado. Token ID: {token_in_db.id}"
                )
                updated_user_in_db = user_to_update
            else:
                update_user_result = await self.user_repository.update(
                    user_id=user_to_update.id, is_verified=True
                )
                if update_user_result.is_failure():
                    logger.error(
                        f"Error al actualizar usuario {user_to_update.id} a verificado: {update_user_result.error()}"
                    )
        ...
        if updated_user_in_db is None:  # Guarda por si una rama no asigna
            logger.error(
                f"updated_user_in_db no fue asignado para token {token_in_db.id} y tipo {expected_token_type}"
            )
            return Failure(DatabaseError(detail="Error interno procesando el token."))

        # 5. Marcar el token como usado
        mark_used_result = await self.user_repository.mark_verification_token_as_used(
            token_in_db.id
        )
...
async def update_user(
    db: AsyncSession, user_id: int, user_update_data: UserUpdate
) -> Result[UserInDB, UserNotFoundError | UserAlreadyExistsError | DatabaseError]:
    """Actualiza un usuario existente utilizando UserService."""
    user_repo = UserRepository(db)
    user_service_instance = UserService(user_repo)
    return await user_service_instance.update_existing_user(
...
async def get_user_by_email(
    ...
    if is_failure(user_in_db_result):
    ...
    try:
        ...
        return Failure(
            DatabaseError(detail=f"Error fetching full user details for email: {email}")
        )
